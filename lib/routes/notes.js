...
router
  //serves all notes from /notes directory if GET req for '/notes'
  .get('/', (req, res, next) => {
    Note.find()
      .then((data) => {
        if (data.length === 0) {
    ...
    Note.findById(req.params.id)
      .then((data) => {
        ...
        //if note is empty of text
          ...
          mistake = {code: 405, error: 'That note exists but is empty. Perhaps you meant to update that note?'};
          next(mistake);
        }else{
  ...
  .get('/search/:prop/:val', (req, res, next) => {
    //square brackets around a variable to use as a key for an object
